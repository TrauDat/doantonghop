...
class MyEditProfile extends Component {
    constructor() {
        super();
        this.state = {
            id: "",
    ...
    isValid = () => {
        ...
        if (fileSize > 2000000) {
            this.setState({
                error: "File size should be less than 200kb",
                loading: false
            });
            return false;
        }
    ...
    handleChange = name => event => {
        this.setState({ error: "" });
        const value =
            name === "photo" || name === "photoBackground" ? event.target.files[0] : event.target.value;

        const fileSize = name === "photo" || name === "photoBackground" ? event.target.files[0].size : 0;
        ...
        return (
            <div className="container">
                ...
                {isAuthenticated().user.role === "admin" &&
                    this.signupForm(name, email, password, about, location)}
